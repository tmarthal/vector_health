 ...
 **/
public class FixedOpenIntHashSet extends AbstractIntSet {
   ...
   */
  public FixedOpenIntHashSet(int initialCapacity) {
   ...
   *                                  maxLoadFactor)</tt>.
   */
  public FixedOpenIntHashSet(int initialCapacity, double minLoadFactor, double maxLoadFactor) {
...
System.out.println("new freeEntries " + freeEntries);    
    trimToSize();
  }
  ...
  @Override
  public Object clone() {
	  FixedOpenIntHashSet copy = (FixedOpenIntHashSet) super.clone();
...
System.out.println("forEachKey called " + table.length);
    for (int i = table.length; i-- > 0;) {
    	System.out.println("forEachKey " + table.length + " " + i);
      if (state[i] == FULL) {
	...
	System.out.println("forEachKey returning true");
  ...
  protected int indexOfKey(int key) {
    ...
    if (decrement == 0) {
	  ...
	  System.out.println("indexOfKey called " + decrement + " " + key + "   + " + length);

for (int tom = 0; tom < table.length; tom++) {
    System.out.println(tom + " " + state[tom] + " " + table[tom]);
}
    // stop if we find a free slot, or if we find the key itself.
    // do skip over removed slots (yes, open addressing is like that...)
    while (i > 0 && state[i] != FREE && (state[i] == REMOVED || table[i] != key)) {
        System.out.println(i + " " + state[i] + " " + table[i]);
      i -= decrement;
      //hashCollisions++;
    }
    if (i < 0) {
...
System.out.println("Trim to size " + newCapacity  + "    " + table.length);    
